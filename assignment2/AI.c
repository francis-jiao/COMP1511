...
playerMove decideMove(Game game) {
    ...
    if (!isValidMove(game, move) || shouldSayUno(game) == TRUE || shouldSayDuo(game) == TRUE || shouldSayTrio(game) == TRUE) {
        ...
        if(isValidMove(game, move)){
            if(turnMoves(game, turn) >= 2){
                playerMove secondMove = pastMove(game, turn, 1);
                if(secondMove.action == SAY_UNO || secondMove.action == SAY_DUO || secondMove.action == SAY_TRIO){
                    endTurn = TRUE;
                }
            }
        }
        ...
        if(endTurn == FALSE){
            if(turnMoves(game, turn) >= 1){
                playerMove firstMove = pastMove(game, turn, 0);
                if(firstMove.action == PLAY_CARD || firstMove.action == DRAW_CARD) {
            ...
            if(canReturn == FALSE){
                if(cardValue(topCard) == 2){
                    move.action = PLAY_CARD;
                    int numCards = handCardCount(game)-1;
                    int escape = FALSE;
                    while(numCards >= 0 && escape == FALSE) {
                        if(cardValue(handCard(game, numCards)) != 2){
                            numCards--;
                        } else {
                            escape = TRUE;
                        }
                    }
                    //if there is no valid cards
                    if(escape == FALSE){
                        move.action = DRAW_CARD;//how to make it draw two cards
                    }
                } else (cardValue(topCard) == DECLARE){
                    move.action = PLAY_CARD;
                    int numCards = handCardCount(game)-1;
                    int escape = FALSE;
                    pastMove
                    while(numCards >= 0 && escape == FALSE) {
                        if(doCardsMatch(handCard(game, numCards),topCard) == FALSE){
                            numCards--;
                        } else {
                            escape = TRUE;
                        }
